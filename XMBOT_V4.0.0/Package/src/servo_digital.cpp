...
uint8_t ServoDigital::TxPacket(uint8_t bID, uint8_t bInstruction, uint8_t bParameterLength)
{
    ...
    board_controller.axServoTxModel();

    for (bCount = 0; bCount < bPacketLength; bCount++)
    {
        board_controller.axServoSendTxByte(gbpTxBuffer[bCount]);
    }

    board_controller.axServoRxModel();
    //for(i=0;i<0x8fff;i++);
...
uint8_t ServoDigital::axSendPosition(uint8_t bID, uint16_t target_pos, uint16_t target_speed)
{
    ...
    gbpParameter[1] = target_pos%256; //Writing Data P_GOAL_POSITION_L
    gbpParameter[2] = target_pos / 256; //Writing Data P_GOAL_POSITION_H
    gbpParameter[3] = target_speed%256; //Writing Data P_GOAL_SPEED_L
    gbpParameter[4] = target_speed/256; //Writing Data P_GOAL_SPEED_H
...
void ServoDigital::executeInstruction(uint8_t *p, uint8_t num)
{
    ...
    board_controller.axServoTxModel();  //Send
    for (i = 0; i < num; i++)
    {
        board_controller.axServoSendTxByte(*p++);
    }

    board_controller.axServoRxModel(); //Rec
    //for(i=0;i<0x8fff;i++);
    if (RxPacket(DEFAULT_RETURN_PACKET_SIZE) == DEFAULT_RETURN_PACKET_SIZE)
        err = OK;
    else
...
{		
    ...
    board_controller.debugPutChar(0XFF); //包头
    board_controller.debugPutChar(Length_L); //长度1
    Check_Sum = Check_Sum + Length_L;
    board_controller.debugPutChar(Length_H); //长度2
    Check_Sum = Check_Sum + Length_H;
    board_controller.debugPutChar(0); //ID =0,代表数字舵机
    board_controller.debugPutChar(2); //代表这是回复信息
    Check_Sum = Check_Sum + 2; //代表指令类型
    board_controller.debugPutChar(command_type);
    Check_Sum = Check_Sum + command_type;
    //发送有效数据
    for (j = 0; j < length; j++)
    {
        board_controller.debugPutChar(*(data + j));
        Check_Sum = Check_Sum + *(data + j);
    }
    Check_Sum = ~Check_Sum; //计算校验和

    board_controller.debugPutChar(Check_Sum);
    Check_Sum = 0;
